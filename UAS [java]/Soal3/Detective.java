...
public class Detective {
    ...
    public String investigateSecretMethodWithParam(
        ...
        for (Method m : methods) {
            Parameter[] pmeter = m.getParameters();
            if (pmeter.length != nParam) {
                continue;
            }
            // else: parameter length is same

            boolean samePtype = true;
            for (Parameter p : pmeter) {
                samePtype = samePtype && p.getType().equals(pType);
            }
            if (!samePtype) {
                continue;
            }
            // else: parameter type is same
            ...
            try {
                m.setAccessible(setAccessible);
                result = m.invoke(obj, params).toString();
            } catch (IllegalAccessException e) {
     ...
     * Method yang di print diurutkan berdasarkan nama method.
     * Format output:
    ...
    public void printAllMethodSignature(Object obj) {
        ...
        for (Method method : methods) {
            ...
            for (int i = 0; i < methodParameters.length; i++) {
                System.out.print(methodParameters[i].getType());
