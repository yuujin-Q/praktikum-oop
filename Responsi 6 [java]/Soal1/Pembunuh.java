...
public class Pembunuh implements Kelas {
    ...
    public void serang(Karakter self, Karakter target) {
        System.out.println(self.getName() + ", sang pembunuh, menyerang " + target.getName() + " dengan " + this.getSenjata());
    ...
    public void gunakanKemampuan(Karakter self, Karakter target) {
        if (target.isHidup()) {
            int amount = Math.min(target.getHealth(), 20);
            System.out.println(self.getName() + " menyerap health " + target.getName() + " sebesar " + amount);
